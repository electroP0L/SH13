...
int guiltSel;
int guiltGuess[13];
int tableCartes[4][8];		//Le gros tableau avec les joueurs et les objets
int b[3];
int goEnabled;
int connectEnabled;
int victoryEnabled;
...
volatile int synchro;		//Synchro entre le thread serveur et le thread principal, 0=attente, 1=fini

void *fn_serveur_tcp(void *arg)		//Thread serveur TCP
{
...
int main(int argc, char ** argv)
{
	...
	int id;
        ...
        strcpy(gServerIpAddress,argv[1]);	//Copie de l'adresse IP du serveur Global
        gServerPort=atoi(argv[2]);			//Définition du port du serveur Global
        strcpy(gClientIpAddress,argv[3]);	//Copie de l'adresse IP du client
        gClientPort=atoi(argv[4]);			//Définition du port du client
        strcpy(gName,argv[5]);				//Copie du nom du joueur

    SDL_Init(SDL_INIT_VIDEO);
	TTF_Init();
	

	//Avec SDL, on crée une fenêtre et un renderer
	...
	joueurSel=-1;		//Ligne de joueur sélectionné par le curseur
	objetSel=-1;		//Colonne d'objet sélectionné par le curseur
	guiltSel=-1;		//on a selectionné une carte qu'on accuse
   ...
   synchro=0;																		//Avant de créer le thread, on met synchro à 0 
   ret = pthread_create ( & thread_serveur_tcp_id, NULL, fn_serveur_tcp, NULL);		//Puis je crée le thread
	...
	if (SDL_PollEvent(&event))
	{
		//printf("un event\n");
        	switch (event.type)
        	{
            		case SDL_QUIT:
                		quit = 1;
                		break;
			case  SDL_MOUSEBUTTONDOWN:				//Si on clique sur la souris
				SDL_GetMouseState( &mx, &my );		//On récupère les coordonnées de la souris
				//printf("mx=%d my=%d\n",mx,my);
				if ((mx<200) && (my<50) && (connectEnabled==1))	//Si on clique sur le bouton connect
				{
				...
				else if ((mx>=0) && (mx<200) && (my>=90) && (my<330))	//Si on clique sur une ligne de joueur
				{
					joueurSel=(my-90)/60;								//On récupère le numéro de la ligne de joueur
					guiltSel=-1;										//On a séléctionné parmis les cartes celles que l'on accuse
				}
				else if ((mx>=200) && (mx<680) && (my>=0) && (my<90))	//Si on clique sur une colonne d'objet
				{
					objetSel=(mx-200)/60;								//On récupère le numéro de la colonne d'objet
					guiltSel=-1;										//On a séléctionné parmis les cartes celles que l'on veut enqueter
				}
        ...
        if (synchro==1)	//Si le thread serveur tcp local a recu un message
        {
                printf("consomme |%s|\n",gbuffer);
		switch (gbuffer[0])
		{
	...
	if (joueurSel!=-1)	//Si on a selectionné une ligne de la grille
	{
	...
	if (objetSel!=-1) 		//Si on a selectionné une colonne de la grille
	{
	...
	if (guiltSel!=-1)	//Si on a selectionné une ligne de la grille
	{
